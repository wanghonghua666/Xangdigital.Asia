...
export const interestsManager = new FirebaseManager('productInterests')
...
// 订阅：添加用户对商品的关注
export const addProductInterest = async ({ productId, email }) => {
  if (!productId || !email) throw new Error('缺少 productId 或 email')
  const normalizedEmail = String(email).trim().toLowerCase()
  const safeId = `${productId}__${normalizedEmail.replace(/[^a-z0-9@._-]/g,'')}`
  // 先查是否已存在，存在则直接返回避免重复写入
  const existing = await interestsManager.getById(safeId)
  if (existing) {
    return { ok: true, duplicate: true }
  }
  // 使用固定ID写入，天然去重
  await setDoc(doc(db, 'productInterests', safeId), {
    productId,
    email: normalizedEmail,
    createdAt: new Date(),
    userAgent: typeof navigator !== 'undefined' ? navigator.userAgent : ''
  })
  interestsManager.clearCache()
  return { ok: true, duplicate: false }
}

// 订阅：获取某商品订阅总数
export const getProductInterestsCount = async (productId) => {
  const list = await interestsManager.getAll({ whereField: 'productId', whereValue: productId, orderByField: null })
  return list.length
}

// 订阅：获取某商品订阅明细
export const getProductInterests = async (productId) => {
  return interestsManager.getAll({ whereField: 'productId', whereValue: productId, orderByField: null })
}
